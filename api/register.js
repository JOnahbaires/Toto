...
<body style="font-family:'Segoe UI',Arial,sans-serif;background:#f5f5f5;padding:20px;">
  <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.1);">
    ...
    <div style="padding:24px;">
      <p style="margin:0 0 16px;">¡Hola ${escapeHtml(nombreAlumno)}!</p>
      <p style="margin:0 0 16px;">Toto ya está listo para estudiar con vos.<br>
      Hacé click acá para arrancar:</p>
      <div style="text-align:center;margin:24px 0;">
        <a href="${link}"
           style="background:#7c6fff;color:#fff;padding:14px 28px;border-radius:8px;text-decoration:none;font-weight:bold;font-size:1rem;display:inline-block;">
          ¡Entrar a Toto! →
  ...
  try {
    const toList = emailAlumno
      ? [emailPadre, emailAlumno]
      : [emailPadre];

    const sends = toList.map((email, i) => fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: i === 0
          ? `${escapeHtml(nombreAlumno)} ya puede empezar a usar Toto`
          : `¡Hola ${escapeHtml(nombreAlumno)}! Tu acceso a Toto está listo`,
        html: i === 0 ? htmlBody : htmlBodyAlumno,
      }),
    }));
    ...
    return res.status(200).json({ ok: true, id: firstData.id });
  } catch (err) {
